...
import { faDiscord, faGithub, faLinkedin } from '@fortawesome/free-brands-svg-icons'
  ...
  return (
    <div className={`py-4 px-5 md:px-20 bg-darkBlue flex transition-shadow duration-700 ${scrollPosition > 1 && "shadow-2xl bg-opacity-80 backdrop-blur-sm"} items-center font-headText sticky top-0 w-full bg-white z-40`}>
        ...
        <div className={`hidden lg:flex ${scrollPosition > 1 && "lg:hidden"}`}>
            <div className='flex items-center mr-5'>
                ...
                <Link target={"_blank"} href={'https://www.linkedin.com/in/jacob-pixler-094348219/'}><FontAwesomeIcon className='text-greyWhite text-4xl px-8' icon={faLinkedin} /></Link>
            ...
            <Link className="bg-techGreen text-darkBlue rounded-md text-xl p-2 flex items-center gap-5 mx-2 hover:cursor-pointer hover:bg-darkBlue hover:text-techGreen border border-techGreen transition duration-200 ease-out hover:ease-in" href={'#about'}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
                </svg>
                ABOUT
            </Link>
            <Link className="bg-techGreen text-darkBlue rounded-md text-xl p-2 flex items-center gap-5 mx-2 hover:cursor-pointer hover:bg-darkBlue hover:text-techGreen border border-techGreen transition duration-200 ease-out hover:ease-in" href={'#projects'}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0112 15a9.065 9.065 0 00-6.23-.693L5 14.5m14.8.8l1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0112 21c-2.773 0-5.491-.235-8.135-.687-1.718-.293-2.3-2.379-1.067-3.61L5 14.5" />
                </svg>
                PROJECTS
            </Link>
            <Link className="bg-darkBlue text-techGreen rounded-md text-xl p-2 flex items-center gap-5 mx-2 hover:cursor-pointer hover:bg-techGreen hover:text-darkBlue border border-techGreen transition duration-200 ease-out hover:ease-in" target={"blank"} href={'https://drive.google.com/file/d/1nMr3QybJfd7rubsAJKub0Kl5zsTjh3Gn/view?usp=sharing'}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
